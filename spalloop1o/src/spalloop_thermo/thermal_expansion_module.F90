MODULE THERMAL_EXPANSION_MODULE
    !=======================================================================
    ! Purpose(s):
    !
    !  Subroutines for thermal expansion in a non-uniform temperature profile. 
    !
  ...
  ! Public Procedures
  public :: TEMP_PROFILE, BETA_TAU_TH, GET_HEAT_FLUX_ALL

 CONTAINS

  SUBROUTINE TEMP_PROFILE(N, heat_flux_input, Temp_out, Temp_in, &
     & thickness_oxide_layer, cycle_no, time_now, dt, &
  ...
  use output_module,            only: tty_lun, out_lun, inp_lun, aux_lun, &
      & gen_lun, prefix
  use waterwall_data_module, only: id_height_now, htc_inner_height_input, &
     & htc_outer_height_input, metal_temperature_model_type

  integer, intent(IN)  :: N, id_high_or_low, cycle_no
  real,    intent(IN)  :: heat_flux_input, Temp_out, Temp_in, time_now, dt
  ...
  if (TRIM(metal_temperature_model_type) == 'design')  then
    ...
    htc_in = htc_inner_height_input(id_height_now)
    htc_out = htc_outer_height_input(id_height_now)
  ...
  ! heat_flux 
  call GET_HEAT_FLUX_ALL(N, Temp_out, Temp_in, &
              & heat_flux)

  call GET_TEMP_ALL(N, heat_flux, Temp_out, Temp_in, &
       & htc_in, htc_out)
 ...
 9 format(a13, 1x, 50(1pe13.6, 1x))
  ...
  heat_flux_out = heat_flux ! htc_out * (Temp_out - Temp(1,1))
  heat_flux_in = heat_flux  ! htc_in * (Temp(N, npr_temp(N)) - Temp_in)
 ...
 32   format('integral_low ', i1, 1x, i2, 1x, 10(1pe13.6, 1x))
  ...
  SUBROUTINE GET_TEMP_ALL(N, heat_flux, gas_temperature, &
     & steam_temperature, htc_in, htc_out)

  ! obtain the temperature at the npr_temp locations
  use solution_data_module, only: rad_int
  use oxide_data_module,    only: growth_ox_temperature, first_oxide_layer, &
          & cond_value
  use solution_data_module, only: rad_temp, Temp, npr_temp, bc_temp
  ...
  ! Argument List
  integer,               intent(IN)  :: N
  real,                  intent(IN)  :: steam_temperature, htc_in, &
         & gas_temperature, htc_out, heat_flux
  ...
  integer :: j, i  
   ...
   Temp_wall_now = steam_temperature + heat_flux * rad_int(1) / &
            & (rad_int(N+1) * htc_in)
   ...
   do j = N, 1, -1
     ...
     bc_temp(1, j) = heat_flux * rad_int(1) / cond_value(j, 1)

   end do

   ! get the temperatures at oxide interfaces and oxide-metal interface
   ! id_layer_start > 2
   do j = N+1, 1, -1
     Temp_int(j) = Temp_wall_now + Temp_int(j) * heat_flux * rad_int(1) !&
   ...
   do i = 1, N
     ...
     ! get temperatures in the oxide
     do j = 1, npr_temp(i)
       Temp(i, j) = Temp_int(i) - bc_temp(1, i) * LOG(rad_int(i) / rad_temp(i, j))
     end do
  ...
  END SUBROUTINE GET_TEMP_ALL

  SUBROUTINE GET_HEAT_FLUX_ALL(N, gas_temperature, &
     & steam_temperature, heat_flux)

  ! obtain the temperature at the npr_temp locations
  use solution_data_module, only: rad_int
  use oxide_data_module,    only: cond_value
  use boiler_data_module,   only: htc_tube_inner, htc_tube_outer
  use waterwall_data_module, only: id_height_now, htc_inner_height_input, &
     & htc_outer_height_input, metal_temperature_model_type
  ...
  real,                  intent(IN)  :: steam_temperature, &
         & gas_temperature
  real,                  intent(OUT)  :: heat_flux

  ! Local Variables
  real :: htc_total, htc_in, htc_out
  integer :: j, i  
  ...
  htc_total = 1.0 / htc_out + rad_int(1)  / (htc_in * rad_int(N+1))
  do j = 1, N
    ! layer j is between [rad_int(j+1) < rad_int(j)]
    htc_total = htc_total + &
      & rad_int(1) * LOG(rad_int(j) / rad_int(j+1)) / cond_value(j, 1)

   end do
   htc_total = 1.0 / htc_total

   heat_flux = htc_total * (gas_temperature - steam_temperature)

   RETURN
     
  END SUBROUTINE GET_HEAT_FLUX_ALL
...
END MODULE THERMAL_EXPANSION_MODULE
