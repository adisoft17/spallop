MODULE BOILER_OPERATION_MODULE
    ...
    ! Purpose(s):
    !
    ! Boiler operation, i.e., load following or load cycling
    !
  ...
  SUBROUTINE BOILER_SCHEDULE_TP_CYCLES(if_cycle)
    ...
    real    :: Temp_ave, total_time, Temp_ave_local, Time_event_now
        ...
        if (no_event_full2low_output(1) < 0)  then
          ...
          do i = 1, mramp_out
            if (SUM(event_time_interval(1:i)) < total_time .and. &
               & event_time_interval(i) > 24.0)  then
 ...
 1      format('no_event_full2low_output = ', 10(1x, i2))
        ...
        Time_event_now = event_time_interval(1)
        event_local_point = 0
        no_event = 0
        ...
        do i=1, no_total_pulse
          ...
          if (k > 4*no_pulse_per_cycle .and. i < no_total_pulse)  then
            ...
            if (Time_Boiler_p(k-4*no_pulse_per_cycle) < Time_event_now .and. &
              ...
              ! next time event
              Time_event_now = Time_event_now + event_time_interval(no_event + 1)
 ...
 210      format('Start event times ', 12(1pe13.6, 1x))
          write(out_lun, 211) Time_Boiler_p(event_start(1)), &
            & (Time_Boiler_p(event_start(i+1)) - Time_Boiler_p(event_start(i)) - &
            & time_boiler_event - time_pulse_event, i = 1, no_event-1)
 211      format('operation_times_actual_between_events ', 12(1pe13.6, 1x))
          write(out_lun, 212) (event_time_interval(i), i = 1, no_event)
 212      format('outage_times_input_between_events ', 12(1pe13.6, 1x))
          write(out_lun, 213) Time_Boiler_p(event_start(1)), &
            & (Time_Boiler_p(event_start(i+1)) - Time_Boiler_p(event_start(i)), &
            & i = 1, no_event-1)
 213      format('outage_times_actual_between_events ', 12(1pe13.6, 1x))
