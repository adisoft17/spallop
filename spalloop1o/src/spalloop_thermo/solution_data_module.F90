MODULE SOLUTION_DATA_MODULE
  ...
  integer, dimension(mramp_out)    :: no_pulse_full2low_output, &
      & no_point_full2low
  integer, dimension(mramp_out)    :: no_event_full2low_output, &
  ...
  ! mave = 3 - sum average
   ...
   ! temp_oxide_high_debug = the oxide temperature at full load for debugging
   real   :: temp_oxide_high_debug

   ! temp_gas_high_load, temp_steam_high_load computed for outage or full 2 low transitions 
   real   :: temp_gas_high_load, temp_steam_high_load, heat_flux_high_load
   ...
   ! three variables for outage, and up to 10 outages - actually changed to be mramp_out outages
   ! strain_elastic_event_out = strain_elastic_event_sp, _mag, _tot
   ! energy_elastic_event_out = energy_elastic_event_sp, _mag, _tot
   ! strain_elastic_event_out(outage, id_now, layer)
   real, dimension(mramp_out, mout_in_f2l)   :: temperature_event_out
   real, dimension(mramp_out, mout_in_f2l, moxide_layer)   :: &
   ...
   real, dimension(mramp_out, moxide_layer)   :: max_energy_elastic_event_out, &
      & max_dist_energy_el_event_out
   ! _pl is for flat plate assumption; biaxial stress
   real, dimension(mramp_out, moxide_layer)   :: max_energy_elastic_event_out_pl, &
   ...
   ! rad_int_event(event, layer) = rad_int(layer)
   real, dimension(mramp_out, moxide_layer)   :: min_strain_elast_event_out, &
       & max_strain_elast_event_out, rad_int_event, oxide_thickness_event
   real, dimension(mramp_out, moxide_layer)   :: min_strain_elast_event_out_pl, &
       & max_strain_elast_event_out_pl

   ! time_event(event)
   real, dimension(mramp_out)                 :: time_event

   ! total number of outages
   integer                  :: no_total_outage
   ...
   ! time_outage = outage times, and id_time_outage points to the time id 'i'
   real, dimension(0:20)   :: time_outage
   integer, dimension(0:20)   :: id_time_outage
   ! no_outages - number of total outages
   integer               :: no_outages
