MODULE HTC_UTILITY_MODULE
 ...
 CONTAINS
  ...
  SUBROUTINE GET_TEMP_HTC_INNER(height_now, Temp_steam_now, press_inner_now, &
    ...
    use waterwall_data_module,  only: duration_ash_deslaging, &
       ...
       & ratio_time_intervals_ash, heat_flux_height, &
       ...
       & metal_temperature_model_type, id_height_now,  &
       & htc_inner_height_input
     ...
     max_iter_htc_temp = 10.0
          ...
          if (if_constant_htc_inner)  then
            ...
            if (TRIM(metal_temperature_model_type) == 'design')  then
              ...
              htc_tube_inner_old =  htc_inner_height_input(id_height_now)
            ...
            ! for htc based on correlations; this needs to be changed
            ! do not use this option for tabulated htc_in
          ...
          HTC_TEMP_LOOP1: do i = 1, max_iter_htc_temp
            ...
            if (if_constant_htc_inner)  then
              ...
              ! still ok since htc here would not depend on T_wall for tabular htc
              ! for htc based on correlations; this needs to be changed
              dhtc_tube_inner_dT_wall = 0.0 ! ACTION 3
 ...
 2              format('nhnl_conv_htc_temp2 ', i2, 1x, 1pe13.6, i1, 1x, 10(1pe13.6, 1x))
  ...
  SUBROUTINE GET_STEAM_TEMP_HTC_INNER(height_now, Temp_steam_now, &
    ...
    ! Arguments
    real, intent(IN)    :: height_now, heat_flux, press_inner_now, &
        & flow_rate_steam_now, Temp_gas_now, Temp_steam_now, time_now, &
        & ash_thickness_now
    real, intent(OUT)   :: htc_inner_value, Temp_wall_new
    ...
    real :: rad_out, rad_in, temp_max_full, a1, &
     ...
     ! steam temperature and htc_inner are KNOWNs
     ...
     if (if_constant_htc_inner)  then
       ...
       htc_tube_inner_now = htc_tube_inner_old

       Temp_steam_new = Temp_steam_now
     ...
     Temp_wall_new = Temp_steam_new + heat_flux * rad_out / &
            & (rad_in * htc_tube_inner_now)
 ...
 1        format('get_ash_thick ', i1, 1x, 3(i2, 1x)) 
  ...
  use waterwall_data_module, only: k_ash, if_ash, &
     & id_height_now, htc_outer_height_input, metal_temperature_model_type
  ...
  integer :: j, i  
  real    :: rad_out, conductance_ash, conductance_metal, htc_tube_outer_now

  rad_out = rad_int(1) + ash_thickness_now  ! outer radius
  
  if (TRIM(metal_temperature_model_type) == 'design')  then

    htc_tube_outer_now =  htc_outer_height_input(id_height_now)

  else

    htc_tube_outer_now = htc_tube_outer

  endif

  ! ash layer
  if (ABS(ash_thickness_now) > 1.0e-9 .and. k_ash > 1.0e-9)  then
    conductance_ash =  LOG(rad_out / rad_int(1)) / k_ash
  else
    conductance_ash = 0.0
  endif

  ! metal layer
  conductance_metal = LOG(rad_int(1) / rad_int(id_layer_start)) / &
     & cond_value(1, 1)

  conductance = conductance_ash + conductance_metal
  ! oxide layers
   ...
   Temp_wall_now = Temp_gas_now - heat_flux * &
      & (conductance * rad_out + 1.0 / htc_tube_outer_now)

   RETURN
     
  END SUBROUTINE GET_CONDUCT_RESISTANCE

  SUBROUTINE OUT_CONDUCT_RESISTANCE(N, id_layer_start, &
     & heat_flux, ash_thickness_now, Temp_gas_now)
  ...
  integer :: j, i  
  real    :: rad_out, conductance_ash, conductance_metal, dTemp_ash, dTemp_out, &
           & dTemp_metal, conductance, htc_tube_outer_now
  ...
  dTemp_ash = heat_flux * conductance_ash * rad_out
  dTemp_metal = heat_flux * conductance_metal * rad_out
  dTemp_out = heat_flux * rad_out / htc_tube_outer_now
  
  write(2, 1) ash_thickness_now, dTemp_out, dTemp_ash, dTemp_metal
1  format('dTemp_ash ', 10(1pe13.6, 1x))

   RETURN
     
  END SUBROUTINE OUT_CONDUCT_RESISTANCE
  ...
  integer :: j, i  
  real    :: rad_out, htc_tube_outer_now
  ...
  Temp_int(1) = Temp_gas_now - heat_flux / htc_tube_outer_now 
