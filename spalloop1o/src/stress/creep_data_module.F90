MODULE CREEP_DATA_MODULE
  ...
  ! stress_eq_conv_height_shutd(height, outage, layer, grid) - 
  ! stress level after convergence for the next time cycle between outages
  real, dimension(100, 0:mramp_out, 10, moxide, mgrid) :: stress_eq_conv_height_shutd
  ...
  real, dimension(100, 0:mramp_out, moxide, mgrid) :: creep_strain_eq_height_shutd
  ...
  type(CREEP_TYPE1), dimension(100, 0:mramp_out, moxide, mgrid) :: &
       & creep_strain_old_height_shutd
  ...
  ! ncreep_steps = ncreep_it - number of steps/iterations to aid in creep convergence
  integer :: ncreep_steps

  ! real, dimension(moxide, mave) :: 

   ! creep_strain_before_exfol(layer, height, outage) layer = 1 - metal, 2 - magnetite
   real, dimension(2, 100, 0:mramp_out) :: creep_strain_eq_before_exfol, &
       & creep_strain_eq_after_exfol, stress_eq_ave_before_exfol, &
       & stress_eq_ave_after_exfol, stress_vm_mean_before_exfol, &
       & stress_vm_mean_after_exfol
   type(CREEP_TYPE1), dimension(2, 100, 0:mramp_out) :: creep_strain_before_exfol, &
       & creep_strain_after_exfol
