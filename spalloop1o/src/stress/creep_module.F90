MODULE CREEP_MODULE
    ...
    ! Purpose(s):
    !
    !   Subroutines for creep effects on stress distribution. 
    !
  ...
  public :: CREEP_STRAIN, CREEP_CONV, CREEP_TERM_S_RAD, CREEP_TERM_U, &
          ...
          & DCREEP_DVM, DCREEP, OUT_CREEP_CONV
 ...
 CONTAINS
  ...
  use creep_utility_module, only: CREEP_NEW_OXIDE
 ...
 2         format('ox_steam_str_cr_corr ', i4, 20(1pe13.6, 1x))
  ...
  use creep_data_module, only: no_intervals_now, operation_id_current, &
       & interval_op_now, creep_strain_old_height_shutd, &
  ...
  use creep_utility_module, only: VON_MISES
 ...
 6    format('stress_eq_zero ', 1pe13.6, 2(1x, i2), 1x, 50(1pe13.6, 1x))
  ...
  CONV_IF: if (convergent)  then

    NORM_IF: if (n_norm > 0) then
    ...
    if (if_outage_ramp_exfol)  then
      ...
      creep_strain_old_height_shutd(jheight_creep, ioutage_creep, :, :)%rad = &
        & creep_strain_old(:, :)%rad
      creep_strain_old_height_shutd(jheight_creep, ioutage_creep, :, :)%hoop = &
        & creep_strain_old(:, :)%hoop
      creep_strain_old_height_shutd(jheight_creep, ioutage_creep, :, :)%ax = &
        & creep_strain_old(:, :)%ax
      creep_strain_eq_height_shutd(jheight_creep, ioutage_creep, :, :) = &
        & creep_strain_eq(:, :)

       do j = 1, 10
         do i = i_cr_start, i_cr_end
           stress_eq_conv_height_shutd(jheight_creep, ioutage_creep, j, i, :) = &
             & stress_eq_conv(j, i, :)
         end do
       end do
       ...
       if (jheight_creep <= 3)  then
         ...
         write(out_lun, 2) ioutage_creep, jheight_creep, &
          & creep_strain_old_height_shutd(jheight_creep, ioutage_creep, N, npr_st(N)/2)%hoop, &
          & creep_strain_eq_height_shutd(jheight_creep, ioutage_creep, N, npr_st(N)/2), &
          & stress_eq_conv_height_shutd(jheight_creep, ioutage_creep, id_low, N, npr_st(N)/2), &
          & stress_eq_conv_height_shutd(jheight_creep, ioutage_creep, id_high, N, npr_st(N)/2), &
          & time_now, iter_st_creep, no_creep_intervals, id_high_or_low

 2       format('creep_height_store ', 2(i2, 1x), 5(1pe13.6, 1x), 3(i3, 1x))
 ...
 13    format('metal_creep_conv ', i1, 1x, i4, 1x, i2, 1x, 50(1pe13.6, 1x))
      ...
      if (stress_eq(i, j) > 1.0e+8)  then
         write(6, 15) ioutage_creep, jheight_creep, &
           & time_now, time_now / 24.0, time_now / &
           & (24.0*365.0), (rad_int(first_oxide_layer) - rad_int(N+1)) / &
           & oxide_thick2si, cycle_no
 15      format('ERROR: divergent; too much creep towards outage=', i2, &
             & ' height=', i2, ' t= ', &
             & 1pe13.6, ' hour ', 1pe13.6, ' days;', 1pe13.6, &
             & ' years;', 1pe13.6, ' [micron]', i6, ' th-cycle')
         stop
      endif
  ...
  if (convergent)  then
    ...
    if (n_norm > 0) then
      ...
      if (no_creep_intervals == ncreep_steps)  then
        ! print only at the end of the operational segment 
        ! (end of ramp up, rump down, 
        ! hold at high temperature, hold of lower temperature)

        if (jheight_creep == 2 .or. jheight_creep == 4 .or. &
          & jheight_creep == 6 .or. jheight_creep == 12 .or. &
          & jheight_creep == 14 .or. jheight_creep == 16)  then

          write(enrg_lun, 18) jheight_creep, ioutage_creep, cycle_no, time_now, &
            & (rad_int(first_oxide_layer) - rad_int(N+1)) / oxide_thick2si, &
            & stress_eq_ave(1, 2), stress_eq_ave(1, 3), stress_eq_ave(1, 1), &
            & stress_eq_ave(N, 2), stress_eq_ave(N, 3), stress_eq_ave(N, 1), &
            & creep_strain_ave(1, 2)%hoop, creep_strain_ave(1, 3)%hoop, &
            & creep_strain_ave(1, 1)%hoop, creep_strain_ave(N, 2)%hoop, &
            & creep_strain_ave(N, 3)%hoop, creep_strain_ave(N, 1)%hoop 
 18    format('vm_height_cr_allt ', 2(i2, 1x), i5, 1x, 70(1pe13.6, 1x))
 ...
 10    format('creep_conv_ave ', 1pe13.6, 1x, 2(i2, 1x), 50(1pe13.6, 1x))
    ...
    if (debug_creep_local .and. jheight_creep == 2)  then
 ...
 16 format('ERROR: stress iterations > max_iterat_solver at t= ', &
  ...
  SUBROUTINE INIT_CREEP(N, kheight, id_next_outage)
  ...
  use parameter_module, only: id_high, id_low, mgrid
  use creep_data_module, only: creep_strain_old, creep_strain_new, &
       & creep_integral_sum, creep_integral_dif, &
       & if_creep_ox_scale, if_creep_metal, creep_strain_eq, &
       & i_cr_start, i_cr_end
  use creep_data_module, only: creep_strain_old_height_shutd, &
       & creep_strain_eq_height_shutd, stress_eq_conv_height_shutd, &
       & stress_eq_conv
  use oxide_data_module,   only: no_oxide, no_layer, &
  ...
  use waterwall_data_module,          only: if_new_height
  use solution_data_module, only:  npr_st
  use output_module,      only: tty_lun, out_lun, aux_lun

  ! Arguments
  integer, intent(IN)  :: N, kheight, id_next_outage
  ...
  if (if_new_height .and. (.not. first_creep_strain) .and. id_next_outage == 1)   &
    & first_creep_strain = .true.

  FIRST_IF: if (first_creep_strain)  then

    ! intialize for each new height to ZERO
   ...
   BETWEEN_OUTAGES: if (if_new_height .and. id_next_outage > 1)  then

     creep_strain_old(:, :)%rad = &
        & creep_strain_old_height_shutd(kheight, id_next_outage - 1, :, :)%rad
     creep_strain_old(:, :)%hoop = &
        & creep_strain_old_height_shutd(kheight, id_next_outage - 1, :, :)%hoop
     creep_strain_old(:, :)%ax = &
        & creep_strain_old_height_shutd(kheight, id_next_outage - 1, :, :)%ax
     creep_strain_eq(:, :) = &
        & creep_strain_eq_height_shutd(kheight, id_next_outage - 1, :, :)

     do i = 1, 10
       do j = i_cr_start, i_cr_end
         stress_eq_conv(i, j, :) =  &
           & stress_eq_conv_height_shutd(kheight, id_next_outage - 1, i, j, :)
       end do
     end do

       if (kheight <= 3)  then

         write(out_lun, 2) id_next_outage, kheight, &
          & creep_strain_old_height_shutd(kheight, id_next_outage-1, N, npr_st(N)/2)%hoop, &
          & creep_strain_eq_height_shutd(kheight, id_next_outage-1, N, npr_st(N)/2), &
          & stress_eq_conv_height_shutd(kheight, id_next_outage-1, id_low, N, npr_st(N)/2), &
          & stress_eq_conv_height_shutd(kheight, id_next_outage-1, id_high, N, npr_st(N)/2)
 2       format('creep_height_retri ', 2(i2, 1x), 50(1pe13.6, 1x))
       endif
