MODULE OXIDE_TEST_MODULE
    ...
    ! Purpose(s):
    !
    ! evaluate simple strain formulations
    !
 ...
 CONTAINS
  ...
  SUBROUTINE CHECK_EXACT_STRAIN_CYL(no_total_layers)
    ...
    use waterwall_data_module,     only: id_height_now
    ...
    real     :: Temp_High, Temp_Low, Temp_gas_now, Temp_steam_now, &
              ...
              & slope_press_steam, time_now, heat_flux_test, &
    ...
    write(out_lun, *) 'test begin'
      ...
      do i = 1, 1
        ...
        ! for this test to work, the htc must be given in a different way
        id_height_now = i

        heat_flux_test = 0.0
        if (heat_flux_test < 0.0) then
          write(6, *) 'ERROR: heat_flux_test < 0'
          stop 
        endif
        ...
        call DRIVER_SOLUTION(no_oxide_layers, TEMP_BOILER_data_no+10, &
              & heat_flux_test, Temp_gas_test, Temp_steam_test, &
