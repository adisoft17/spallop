MODULE INDUCED_STRESS_MODULE
    ...
    ! Purpose(s):
    !
    !   Subroutines for oxided-induced stresses. 
    !
 ...
 3    format('a_metal2spinel ', i2, 1x, 20(1pe13.6, ' '))
  ...
  use boiler_data_module, only: tube_outer_radius, tube_thickness, &
      & Time_Boiler_p, oxide_thickness, TEMP_BOILER_data_no
  ...
  if (i_print <= TEMP_BOILER_data_no)  then
  ...
  ! reset if new height
  if (if_new_height .and. (.not. first_set_ox_strains))  &
     & first_set_ox_strains = .true.

  if (first_set_ox_strains)  then

    ox_strain_hoop_old = 0.0
    first_set_ox_strains = .false.
    ! write(out_lun, *) 'first_set_ox_strains ', cycle_no; setting done at cycle =2

  endif 

  ! reset first_set_ox_strains after last cycle at each height
