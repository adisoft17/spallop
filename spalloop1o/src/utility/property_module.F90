MODULE PROPERTY_MODULE
    ...
    ! Purpose(s):
    ! 
    !  functions, operands on tabular data
    !
  ...
  public :: LINEAR_PROPERTY_SIMPLE, ALLOCATE_AVERAGE_PROPERTY, &
         & PROPERTY_SCALE_AVE, OPERAND_ARRAY, OPERAND_TWO_ARRAY
 ...
 31 format('th_exp_oxide_ave', 2(1x, 1pe13.6))
  ...
  SUBROUTINE OPERAND_ARRAY(dim1, dim2, property, id1, id2, &
    ...
    ! get the maximum number of the property
    if (id2-id1+1 < 1)  then
      write(tty_lun, *) 'ERROR: OPERAND_ARRAY id2-id1+1 < 1', id1, id2
      write(tty_lun, *) property(dim1:dim2), TRIM(operation_type)
      STOP
    endif
