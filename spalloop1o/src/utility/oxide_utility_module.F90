MODULE OXIDE_UTILITY_MODULE
 ...
 SUBROUTINE STORED_EL_ENERGY_OX(ilayer, elastic_energy, &
    ...
    ! Purpose(s):
    !
    !  specific elastic strain energy per unit circumferential area 
    !    in the oxide layer
    !  strain energy density = 1/2 than specific elastic strain energy
    !  "stored 
    ! 
    !=======================================================================
    use parameter_module,   only: moxide, mgrid, mave
    ...
    use oxide_data_module,  only: temp_kelvin_unit, th_exp_temp, &
    ...
    real   :: r, rp1, delta_energy_hoop, delta_energy_rad, &
     & delta_energy_axial, area, radius_mean, delta_energy_dilat_j, &
     & delta_energy_dilat_jp1, delta_energy_dilat, dilatat_el_energy, &
     & delta_energy, strain_energy_density_ave
    integer :: k, j, loc1
    real, dimension(mgrid)  :: strain_energy_local
    ...
    do j = npr_st(ilayer) - 1, 1, -1
      ...
      ! excluding the pi factor; 1/2 since integral over area is counted twice
      ! 1/2 factor since the function should be 0.5 * (sigma*strain ..)
      ! 2 factors comes from Integral_area(W)= 2*pi*Integral_radius(W*r)
      ! total factor 2*0.5*0.5=0.5
      delta_energy_hoop = 0.5 * (stress_hoop(ilayer, j) * &
          & strain_hoop_el(ilayer, j) * r + &
          & stress_hoop(ilayer, j+1) * strain_hoop_el(ilayer, j+1) * &
          & rp1)
      delta_energy_rad = 0.5 * (stress_rad(ilayer, j) * &
          & strain_rad_el(ilayer, j) * r + &
          & stress_rad(ilayer, j+1) * strain_rad_el(ilayer, j+1) * &
          & rp1)
      delta_energy_axial = 0.5 * (stress_axial(ilayer, j) * &
    ...
    radius_mean = 0.5 * (rad_st(ilayer, 1) + &
          & rad_st(ilayer, npr_st(ilayer)))

    ! strain energy density = is 1/2 than specific elastic strain energy for biaxial
    ! strain energy density = specific elastic strain energy for plain strain state

    ! so far, elastic_energy = Integral_radius[r*0.5*(sigma*strain ..)]

    elastic_energy = elastic_energy / (2.0 * radius_mean)

    ! this energy for pure elastic case is
    ! E * (tau_metal - tau_oxide)/ (1 - Poiss)
    ! notice that it does not have the 1/2 factor

    ! elastic_energy = 0.5 * elastic_energy / transveral_length before 080811

    ! check this
    dilatat_el_energy = dilatat_el_energy / (2.0 * radius_mean)
 ...
 SUBROUTINE CHECK_STORED_EL_ENERGY_OX(ilayer, elastic_energy, &
        & distortion_el_energy)
    !=======================================================================
    ! Purpose(s):
    !
    ...
    k = 3
    do j = 1, npr_st(ilayer)
      strain_energy_local(j) = 0.5 * (stress_hoop(ilayer, j) * &
          & strain_hoop_el(ilayer, j) + stress_rad(ilayer, j) * &
          & strain_rad_el(ilayer, j) + stress_axial(ilayer, j) * &
          & strain_axial_el(ilayer, j))
    end do

    call OPERAND_ARRAY(1, mgrid, strain_energy_local, 1, npr_st(ilayer), &
       & operation_type(k), strain_energy_density_ave, loc1)

    do j = 1, npr_st(ilayer)
      strain_energy_local(j) = stress_hoop(ilayer, j)**2 + &
        & stress_rad(ilayer, j)**2 + stress_axial(ilayer, j)**2 - &
        & 2.0 * poisson_ratio(ilayer) * &
        & (stress_hoop(ilayer, j) * stress_rad(ilayer, j) + &
        & stress_hoop(ilayer, j) * stress_axial(ilayer, j) + &
        & stress_axial(ilayer, j) * stress_rad(ilayer, j))
    end do

    strain_energy_local = strain_energy_local / &
          & (2.0 * Youngs_modul(ilayer, 1))
    ...
    elastic_energy = strain_energy_density_ave * (rad_st(ilayer, 1) - &
           & rad_st(ilayer, npr_st(ilayer)))
  ...
  END SUBROUTINE CHECK_STORED_EL_ENERGY_OX
  ...
  SUBROUTINE STORED_EL_ENERGY_OX_PLATE(ilayer, elastic_energy, &
    ...
    ! Purpose(s):
    !
    !  specific elastic strain energy per unit length in the oxide layer
    !     using pure elastic bi-axial approximation
    !  strain energy density = 1/2 than specific elastic strain energy
    ! 
    !=======================================================================
    ...
    use output_module,      only: tty_lun, out_lun, aux_lun
    use oxide_data_module,  only: poisson_ratio, Youngs_modul
    use solution_data_module, only:  npr_st, rad_st, strain_th_plate_ave
    ...
    ! strain energy density = is 1/2 than specific elastic strain energy
    ! check dilatat_el_energy to be consistent with this 1/2 factor; maybe we need a 1/2 factor since the original expressions might have been valid for energy densities rather than specific energies
    ! before 080811 used to be
    !               & (1.0 * (1.0 - poisson_ratio(ilayer)))
    ! should strain_th_plate_ave be used instead of strain_hoop_el_ave to get the energy?
    ! yes since to get strain_hoop_el_ave the stress-strain equation needs to be solved
    ! notice that it does not have the 1/2 factor since 
    ! the integral expression does not have it
    elastic_energy = (strain_th_plate_ave(ilayer, 3)**2) * &
                   & Youngs_modul(ilayer, 1) / &
  ...
  use solution_data_module, only: rad_temp, Temp, npr_temp, &
      & npr_st, rad_st, eps0, &
      & rad_int, stress_hoop_th_ave, strain_th_plate_ave, &
  ...
  do i = 2, N
    if (if_ave_scale)  then
      ...
      Youngs(i) = Youngs_modul(i, 1)   ! only for average
  ...
  real  :: htc_in, htc_out, poisson_star, Youngs_star
  real, dimension(N)   :: bc, cc
  character(LEN = 80), dimension(mave), save    :: operation_type
  logical  :: if_debug_ave1 = .false.
  ...
  if (if_ave_scale)  then
    ...
    i = N  ! valid only when N=2; get the stresses as for flat plane
...
!      & 6(F8.4, 1x), 1x, 10(F6.1, 1x))
 ...
 105        format(40(a))
         ...
         if (enrg_lun > 0)  then
           write(enrg_lun, 106) 
 106        format(' d_f t_f d_p t_p uel_sp_f uel_mag_f uel_maghe_f ', &
            &   'uel_tot_f uel_sp_p uel_mag_p uel_maghe_p uel_tot_p')
           write(enrg_dist_lun, 107) 
 107        format(' d_f t_f d_p t_p uel_dist_sp_f uel_dist_mag_f ', &
            &   'uel_dist_maghe_f uel_dist_tot_f uel_dist_sp_p ', &
            &   'uel_dist_mag_p uel_dist_maghe_p uel_dist_tot_p')
           write(enrg_dil_lun, 108) 
 108        format(' d_f t_f d_p t_p uel_dil_sp_f uel_dil_mag_f ', &
            &   'uel_dil_maghe_f uel_dil_tot_f uel_dil_sp_p ', &
            &   'uel_dil_mag_p uel_dil_maghe_p uel_dil_tot_p')
         endif
 ...
 16     format(a35, i4, 1x, 2(1pe13.6, 1x), 30(1pe11.4, 1x))
        ...
        if (k==1)  then
          write(aux_lun, *) ch_int2, 'id_time Time t_ox r_cm e_hg s_h s_r s_z'
        endif
 ...
 50     format(a45, &
  ...
  SUBROUTINE OUTPUT_STRAIN_STRESS_F2L(i_oxide_index, f2l_index, &
     & k_height, id_next_outage, thickness_oxide_layer, time_now,  &
     & temp_gas_now, temp_steam_now, &
     & N, if_ave_scale, if_outage_ramp_exfol, if_new_output)
    !=======================================================================
    ! Purpose(s):
    !
    !  PRINT out the strain, stress, temp at the interfaces metal-flue gas, 
    ...
    !=======================================================================
    use parameter_module,   only: mstep, moxide, oxide_thick2si, &
          & id_low, id_high, mave, ten_p3, mramp, zero, mramp_out, mgrid
    ...
    use solution_data_module, only: time_event, rad_int_event, &
      & oxide_thickness_event
    ...
    use solution_data_module, only:  strain_hoop_el, strain_rad_el, &
          & strain_axial_el, rad_st
    use property_module, only: OPERAND_ARRAY
    use oxide_stress_data_module, only: imagnetite

  ! arguments
  integer, intent(IN)  :: N, i_oxide_index, f2l_index  ! i - time index; k - f2l transition index
  integer, intent(IN)  :: k_height, id_next_outage
  real, dimension(moxide), intent(IN)  :: thickness_oxide_layer
  real, intent(IN)  :: time_now, temp_gas_now, temp_steam_now
  logical, intent(IN)  :: if_ave_scale, if_new_output, if_outage_ramp_exfol

    ! Local Variables
    integer  :: j, step_cycle, nstart, m, i, k, ilayer, loc1
    real     :: Temp_High, Temp_Low, dtime_f2l_now, &
              ...
              & dist_el_energy_tot, stored_el_energy_total_pl, &
              & dist_el_energy_tot_pl
    real, save    :: temp_oxide_high, temp_oxide_low
    real, dimension(3)     :: fr, e_check, energy
    real, dimension(mstep)     :: e_ave, energy_ave
    character(LEN = 80)    :: mode_stress = 'biaxial_plane'
    real, dimension(moxide) :: e_th_scale, thickness_layer
    real, dimension(moxide) :: dtemp, dstrain_th, &
          & dstrain_hoop, dstress_hoop, tau_th_out
    real, dimension(mramp_out)  :: press_full_int
    ...
    real, dimension(N)  :: stored_dist_el_energy, &
       & stored_dist_el_energy_plate
    real, dimension(N)  :: stored_el_energy, &
       & stored_el_energy_plate

    real, dimension(mgrid)  :: strain_energy_local
    real, dimension(3)      :: strain_energy_density_ave
    character(LEN = 80), dimension(mave), save    :: operation_type
     ...
     if (if_new_output)  then
        ...
        if (legend_first)  then
          ...
          if (f2l_lun < 0)  then
            ...
            write(tty_lun, *) 'ERROR: f2l_lun must be > 0'
            STOP
 ...
 84     format(a16, i4, 1x, 2(1pe13.6, 1x), &
     ...
     do j = first_oxide_layer, N
       ...
       dist_el_energy_tot = dist_el_energy_tot + stored_dist_el_energy(j)
            ...
            if (id_next_outage == 2 .and. k_height == 2)  then
              ! debug magnetite at height 2
              write(out_lun, 12) id_next_outage, k_height, stored_el_energy(first_oxide_layer: N)
  12          format('check_max_energy_elastic_event2 ', 2(i2, 1x), 10(1pe13.6, 1x))
            endif
 ...
 76     format(a17, i4, 1x, 2(1pe13.6, 1x), &
      ...
      if (if_outage_ramp_exfol)  then
        ! if (no_pulse_full2low_output(f2l_index) == 0)  then
        ...
        id_outage = id_next_outage

        if (time_now <= 1.0e-4)  then

          ! id_now - stores the number of step in the current outage
          ! this actually the first point on the ramp down; it is not at high load
          id_now = 1

          write(out_lun, *) 'start_of_outage ', id_outage

          if (id_outage == 1 .and. k_height == 1)  then

            ! temp_oxide_high = (Temp(N, npr_temp(N)) + &
            !  & Temp(no_metal_layers, npr_temp(no_metal_layers))) / 2.0 
            temp_oxide_low = (Temp(N, npr_temp(N)) + &
              & Temp(no_metal_layers, npr_temp(no_metal_layers))) / 2.0 
 ...
 3      format('check index ', 10(i3, 1x))
     ...
     ! obtain the elastic storage energy; copy this one here, too
     stored_el_energy_total = 0.0
     dist_el_energy_tot = 0.0
     do j = first_oxide_layer, N
       call STORED_EL_ENERGY_OX(j, stored_el_energy(j), &
       ...
       dist_el_energy_tot = dist_el_energy_tot + stored_dist_el_energy(j)
            ...
            if (id_next_outage == 2 .and. (k_height == 1 .or. k_height == 2))  then
              ! debug magnetite at height 2
              write(out_lun, 15) id_next_outage, k_height, stored_el_energy(first_oxide_layer: N)
  15          format('check_max_energy_elastic_event2a ', 2(i2, 1x), 10(1pe13.6, 1x))
            endif
     ...
     do j = first_oxide_layer, N
       ...
       dist_el_energy_tot_pl = dist_el_energy_tot_pl + stored_dist_el_energy_plate(j)
        ...
        if (id_outage == 2 .and. (k_height == 1 .or. k_height == 2))  then

          ! debug magnetite at height 2
          write(out_lun, 13) id_outage, k_height, id_now, &
              &  stored_el_energy(first_oxide_layer: N), &
              &  (rad_st(ilayer, 1) - rad_st(ilayer, npr_st(ilayer)), &
              &  ilayer = first_oxide_layer, N)
  13      format('check_max_energy_elastic_event3 ', 3(i2, 1x), 10(1pe13.6, 1x))
    ...
    call OPERAND_ARRAY(1, mgrid, strain_energy_local, 1, npr_st(ilayer), &
       & operation_type(3), strain_energy_density_ave(ilayer), loc1)
    ...
    call OPERAND_ARRAY(1, mgrid, strain_energy_local, 1, npr_st(ilayer), &
       & operation_type(3), strain_energy_density_ave(ilayer), loc1)
          ...
          write(out_lun, 14)  id_outage, k_height, id_now, strain_energy_density_ave(2:3)
  14      format('check_max_energy_elastic_event4 ', 3(i2, 1x), 10(1pe13.6, 1x))
        ...
        if (id_outage == 1 .and. k_height == 1)  then
          ...
          ! tau_th_st(1, npr_st(1)) - tau_th_st(i, :)
          if (ABS(temp_oxide_high_debug - temp_oxide_low) > 1.0e-7)  then
            ! tau_th_out(1:N) = tau_th_st(1:N, npr_st(1:N)) / &
            tau_th_out(first_oxide_layer:N) = &
              & strain_th_plate_ave(first_oxide_layer:N, 3) / &
              & ((th_exp_coeff(1, 1) - th_exp_coeff(first_oxide_layer:N, 1)) * &
              & (temp_oxide_high_debug - temp_oxide_low))

          else
            ! tau_th_out(1:N) = -1.0
            ! exclude this point
            temp_oxide_low = temp_oxide_high_debug - 1.0e-7
            tau_th_out(first_oxide_layer:N) = &
              & strain_th_plate_ave(first_oxide_layer:N, 3) / &
              & ((th_exp_coeff(1, 1) - th_exp_coeff(first_oxide_layer:N, 1)) * &
              & (temp_oxide_high_debug - temp_oxide_low))
          ...
          if (id_now > 1)  then
          ! print the stored energy in J/m
          write(out_lun, 2) id_now, 60.0 * time_now, &
            & oxide_thickness(i_oxide_index), temp_gas_now, Temp(1, 1), &
            & Temp(no_metal_layers, npr_temp(no_metal_layers)), &
            & Temp(N, npr_temp(N)), temp_steam_now, &
            & temp_oxide_high_debug, temp_oxide_low, &
            & temp_oxide_high_debug - temp_oxide_low, &
            & (tau_th_st(j, npr_st(j)) / (th_exp_coeff(j, 1) * &
            & (temp_oxide_high_debug - temp_oxide_low)), &
            & tau_th_out(j), &
            & (tau_th_st(1, npr_st(1)) - tau_th_st(j, npr_st(j)))*ten_p3, &
            & strain_hoop_el_ave(j, 3)*ten_p3, &
            & energy_elastic_event_out(id_outage, id_now, j), &
            & energy_elastic_event_out_plate(id_outage, id_now, j), &
            & j=first_oxide_layer, N)
          endif
 2     format('test_cte_isoth ', i2, 1x, 50(1pe13.6, 1x))

     if (id_next_outage == 1 .and. k_height == 1)  then

       if (stored_el_energy_total > 0.0)  then

         write(out_lun, 26)  id_now, 60.0 * time_now, &
          & stored_el_energy(imagnetite)/stored_el_energy_total
 26      format('otored_el_energy_mag_over_total ', i2, 1x, 10(1pe13.6, 1x))
        ...
        if (id_now == no_out_f2l + 1)  then
          ...
          if (if_outage_ramp_exfol)  then
            ...
            write(out_lun, 6) id_outage, k_height, id_next_outage, &
               & id_now, no_out_f2l + 1, &
               & oxide_thickness(i_oxide_index), &
               & time_full2low(f2l_index, mramp)
 6          format('good_max_energy_elastic_event_out ', 5(i2, 1x), 20(1pe13.6, 1x))
     ...
     if (id_next_outage == 1 .and. k_height == 1)  then
       ...
       if (stored_el_energy_total > 0.0)  then
         ...
         write(out_lun, 25)  &
           & thickness_oxide_layer(imagnetite)/SUM(thickness_oxide_layer(2:N)), &
           & stored_el_energy(imagnetite)/stored_el_energy_total, &
           & (stored_el_energy(imagnetite)/stored_el_energy_total) / &
           & (thickness_oxide_layer(imagnetite)/SUM(thickness_oxide_layer(2:N))) 
 25      format('stored_el_energy_mag_over_total ', 10(1pe13.6, 1x))
            ...
            write(out_lun, 7) id_outage, k_height, id_next_outage, &
               & id_now, no_out_f2l + 1, &
               & oxide_thickness(i_oxide_index), &
               & time_full2low(f2l_index, mramp)
 7          format('bad1_max_energy_elastic_event_out ', 5(i2, 1x), 20(1pe13.6, 1x))

          endif

        endif

        END_OUTAGE_CURRENT: if (id_now == no_out_f2l + 1)  then
          ! end of this current outage was now reached

          ! store final number of outages
          ! no_total_outage = id_outage no need

          ! store the dimesions to get oxide thickness and geometry
          rad_int_event(id_outage, 1:N+1) = rad_int(1:N+1)
          ! write(out_lun, *) 'rad_int_debug ', id_outage, rad_int(2), (rad_int(k)-rad_int(2), k = 3, N+1)
          oxide_thickness_event(id_outage, 1:N+1) = thickness_oxide_layer(1:N)
          time_event(id_outage) = Time_Boiler_p(i_oxide_index) ! time_now
         ...
         & MAXVAL(energy_elastic_event_out(id_outage, 1:no_out_f2l + 1, k))
            ...
            if (k == 3 .and. id_outage == 2 .and. (k_height == 1 .or. k_height == 2))  then
              ! debug magnetite at height 2
              write(out_lun, 11) id_outage, k_height, max_energy_elastic_event(first_oxide_layer: N)
              write(out_lun, 11) first_oxide_layer, N, max_energy_elastic_event(first_oxide_layer: N)
  11          format('check_max_energy_elastic_event1 ', 2(i2, 1x), 10(1pe13.6, 1x))
            endif
         ...
         & MINVAL(strain_elastic_event_out_g(j, 1:no_out_f2l + 1, k))
          ...
          write(tty_lun, 4) id_outage, k_height, &
               & oxide_thickness(i_oxide_index), &
               & time_full2low(f2l_index, mramp), &
               & dtime_f2l_now * no_out_f2l, dtime_f2l_now
          write(out_lun, 4) id_outage, k_height, &
               & oxide_thickness(i_oxide_index), &
               & time_full2low(f2l_index, mramp), &
               & dtime_f2l_now * no_out_f2l, dtime_f2l_now
          ...
          if (gen_lun > 0)  then
            write(gen_lun, 4) id_outage, k_height, &
               & oxide_thickness(i_oxide_index), &
               & time_full2low(f2l_index, mramp), &
               & dtime_f2l_now * no_out_f2l, dtime_f2l_now
          endif

 4        format('outage ', i2, ' at h= ', i2, ' d_ox= ', &
               & 1pe10.3, ' t= ', 1pe10.3, &
               & ' transit dt= ', 1pe10.3, &
               & ' dt= ', 1pe10.3)  ! , 1(i4, 1x))

        endif END_OUTAGE_CURRENT

        ! make sure that all the f2l transitions are complete before printing, 
        ! including the last transition
        END_OUTAGE_ALL: if (id_outage == no_f2l_event_out .and. &
         ...
         if (N == 2) then  ! one-oxide layer
           ...
           write(f2l_lun, 91) (' T'//ch_index(j)//'_ox_met', &
            & ' uel'//ch_index(j)//'_tot'// &
            & ' eh'//ch_index(j)//'_tot', j = 1, MIN(9, no_f2l_event_out)), &
            & (' T'//'1'//ch_index(j-10)//'_ox_met', &
            & ' uel'//'1'//ch_index(j-10)//'_tot'// &
            & ' eh'//'1'//ch_index(j-10)//'_tot', &
           ...
           write(f2l_lun, 91) (' T'//ch_index(j)//'_ox_met', &
            ...
            & ' eh'//ch_index(j)//'_sp_ave', &
            & ' uel'//ch_index(j)//'_mag',&
            & ' eh'//ch_index(j)//'_mag_ave', j = 1, MIN(9, no_f2l_event_out)), &
            &  (' T'//'1'//ch_index(j-10)//'_ox_met', &
            & ' uel'//'1'//ch_index(j-10)//'_tot'// &
            & ' uel'//'1'//ch_index(j-10)//'_sp'// &
            & ' eh'//'1'//ch_index(j-10)//'_sp_ave', &
            & ' uel'//'1'//ch_index(j-10)//'_mag', &
            & ' eh'//'1'//ch_index(j-10)//'_mag_ave', &
            & j = 1+MIN(9, no_f2l_event_out), no_f2l_event_out)
           ...
           write(f2l_lun, 91) (' T'//ch_index(j)//'_ox_met', &
            ...
            & ' eh'//ch_index(j)//'_mag23he_ave', j = 1, MIN(9, no_f2l_event_out)), &
            & (' T'//'1'//ch_index(j-10)//'_ox_met', &
            & ' uel'//'1'//ch_index(j-10)//'_tot'// &
            & ' uel'//'1'//ch_index(j-10)//'_sp'//&
            & ' eh'//'1'//ch_index(j-10)//'_sp_ave', &
            & ' uel'//'1'//ch_index(j-10)//'_mag',&
            & ' eh'//'1'//ch_index(j-10)//'_mag_ave', & 
            & ' uel'//'1'//ch_index(j-10)//'_maghe',&
            & ' eh'//'1'//ch_index(j-10)//'_maghe_ave', &
            & ' uel'//'1'//ch_index(j-10)//'_mag23he',&
            & ' eh'//'1'//ch_index(j-10)//'_mag23he_ave', &
            & j = 1+MIN(9, no_f2l_event_out), no_f2l_event_out)
 ...
 91      format(' time', 150(a))
          ...
          write(tty_lun, 19) time_full2low(f2l_index, 1), &
               & time_full2low(f2l_index, mramp), &
               & dtime_f2l_now * no_out_f2l, dtime_f2l_now
          write(out_lun, 19) time_full2low(f2l_index, 1), &
               & time_full2low(f2l_index, mramp), &
               & dtime_f2l_now * no_out_f2l, dtime_f2l_now

 19        format('last_outage from time = ', &
               & 1pe13.6, 'to time= ', 1pe13.6, &
  ...
  use parameter_module, only: moxide_layer, mramp_out
  ...
  real, intent(IN), dimension(mramp_out, mout_in_f2l)   :: temperature
  real, intent(IN), dimension(mramp_out, mout_in_f2l, moxide_layer)   :: &
       & energy_elastic, strain_elastic
