...
2       format(a,1x, a)
  ...
  SUBROUTINE READ_INPUT ()
    ...
    use waterwall_input_module,   only: WATERWALL_INPUT, WATERWALL_INIT
    use blockage_module,          only: BLOCKAGE_INPUT
...
17     format (/,9x,'FATAL: Error in WATERWALL namelist fatal flag = ', L4/)
    ...
    rewind inp_lun
    fatal = .false.
    call BLOCKAGE_INPUT (fatal)
    if (fatal)  then
       write (out_lun, 34) fatal
       write (tty_lun, 34) fatal
34     format (/,9x,'FATAL: Error in BLOCKAGE namelist fatal flag = ', L4/)
       stop
    endif
 ...
 1  format('htc_htc_htc_TgFull_TstFull_hf_TgP_TstP_hf ', 20(1pe13.6, 1x))
  ...
  real, dimension(30)   ::   outage_time_interval
...
15          format (/' DONE Reading Boiler Namelist ...')
      ...
      ! simplified input for outage events
      if (outage_time_interval(1) > 0.0)  then
        if (outage_time_interval(2) < 24.0)  then
          ! only one interval is given, assume constant outage to accomodate legacy
          outage_time_interval(2:30) = outage_time_interval(1)
        endif
 ...
 209    format('time_ampl ', i4, 1x, 10(1pe13.6, 1x))
  ...
  use oxide_data_module,       only: activ_energy_oxide, const_energy_oxide,  &
      ...
      & oxide_rate_log, oxide_rate_1temp, oxide_thick_uf, micron2m, &
...
15          format (/' DONE Reading OXIDE_GROWTH Namelist ...')
       ...
       micron2m = 1.0e-6 / oxide_thick_uf
